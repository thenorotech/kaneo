...
RUN sed -i 's/"prepare": "husky"/"prepare": ""/g' package.json && \
  corepack enable && \
  corepack prepare pnpm@10.32.1 --activate && \
  HUSKY=0 NODE_ENV=production pnpm install --prod --frozen-lockfile --no-optional --config.public-hoist-pattern[]=*
...
RUN sed -i 's/\r$//' /docker-entrypoint.d/env.sh /usr/local/bin/kaneo-entrypoint.sh && \
    chmod +x /docker-entrypoint.d/env.sh /usr/local/bin/kaneo-entrypoint.sh
