          ...
          <div className="mx-auto max-w-5xl space-y-6">
            ...
            {isLoading ? (
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {sprints?.map((sprint) => (
                  <div key={sprint.id} className="rounded-xl border bg-card text-card-foreground shadow p-4">
                    <h3 className="font-semibold leading-none tracking-tight mb-2">{sprint.name}</h3>
                    <p className="text-sm text-muted-foreground mb-4">{sprint.goal || "No goal specified"}</p>
                ...
                {!sprints?.length && (
                  <div className="col-span-full py-8 text-center text-muted-foreground border border-dashed rounded-xl">
                    No sprints found. Ensure your charter is approved to auto-generate sprints, or create one manually.
                  </div>
                )}
              </div>
            )}
