...
export default function CharterApprovalWorkflow({
  projectId,
}: CharterApprovalWorkflowProps) {
    ...
    return (
      <div className="rounded-md border border-green-200 bg-green-50 p-4 space-y-4">
        <p className="text-sm text-green-800 font-medium">
          This charter has been fully approved.
        </p>
        <Button
          onClick={() => autoCreate({ projectId })}
  ...
  return (
    <div className="space-y-4 rounded-md border p-4 bg-card">
      ...
      <p className="text-sm text-muted-foreground">
        Review the charter and provide your approval or return it with
        observations.
      </p>

      <div className="space-y-2">
        <label htmlFor="comments" className="text-sm font-medium">
          Observations (if returning)
        </label>
        <Textarea
          id="comments"
          value={comment}
